...
function EditSalePage() {
    ...
    const totals = useMemo(() => {
        ...
        for (const it of items) {
            ...
            subtotal += lineAfter;
            totalTax +=
                (lineAfter * (Number.isFinite(rate) ? rate : 0)) / 100;
        }
    ...
    const addInstallment = () => {
        setInstallments((prev) => [
            ...
            {
                _key: nextKey(),
                due_date: saleDate || new Date().toISOString().split("T")[0],
                amount: "0",
            },
        ]);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (saleType === "credit" && !customerId) {
            toast("Client requis", {
                description:
                    "Une vente à crédit nécessite un client avec limite de crédit.",
            });
            return;
        }
        ...
        try {
            ...
            router.push(
                `/organisation/${orgId}/inventory/sales/${saleId}`
            );
        } catch (err) {
            toast.error("Impossible de mettre à jour la vente", {
            ...
            <div className="container mx-auto p-6">
                <Card>
                    <CardContent className="pt-6 space-y-3">
                        <p className="text-sm">
                            Cette vente ne peut plus être modifiée (statut :{" "}
                            <span className="font-medium">
                                {sale.status_display}
                            </span>
                            ). Seuls les brouillons sont éditables.
                        </p>
                        <Button
                            variant="outline"
                            onClick={() =>
    ...
    return (
        <FormPageLayout
            ...
            subtitle="Modifiez le brouillon avant de le finaliser pour décompter le stock."
            backLink={`/organisation/${orgId}/inventory/sales/${saleId}`}
            sidebar={
                <Card>
                    <CardHeader>
                        ...
                        {saleType === "credit" &&
                            installments.length > 0 && (
                                <div className="pt-3 border-t space-y-1">
                                    ...
                                    {Math.abs(
                                        installmentsTotal - totals.total
                                    ) > 0.01 && (
                                        <p className="text-xs text-amber-600">
                                            ⚠ Les échéances doivent couvrir
                                            le total
                                        </p>
                                    )}
                                </div>
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                {[
                                    ...
                                    },
                                    {
                                        ...
                                        icon: CreditCard,
                                        description:
                                            "Avec échéancier et contrôle du crédit client",
                            ...
                            <div className="space-y-3 pt-4 border-t">
                                ...
                                {installments.length === 0 ? (
                                    <p className="text-xs text-muted-foreground italic">
                                        Sans échéance définie, le total est
                                        dû à la date d&apos;échéance finale.
                                    </p>
                                ) : (
                                    <div className="space-y-2">
